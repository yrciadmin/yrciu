...
const Instructors: React.FC = () => {
  return (
    <section className="px-[16px] pt-[1vmax] pb-[6.6vmax] md:px-[4vw]">
      <Heading className="mb-[60px] font-medium fluid-text-6xl">Meet our talented instructors</Heading>
      <div className="grid grid-cols-1 items-stretch gap-[20px] sm:grid-cols-2 md:grid-cols-3 lgr:grid-cols-5">
        {instructors.map((instructor) => (
          <Link 
            ...
            passHref 
          >
            <a className="flex flex-col rounded-b-[5px] bg-rb-white text-rb-black no-underline shadow-[0px_0px_5px_rgba(0,0,0,.15)] hover:no-underline">
              <div className='aspec aspec relative mb-[9%] aspect-[4/3] shrink-0'>
                <Image className="absolute h-full w-full object-cover" fill={true} src={instructor.image} alt={`${instructor.title}`} />
              </div>
              <div className="flex h-full flex-col px-[6%] pb-[6%]">
                <h3 className="mb-4 font-normal leading-[1.25] fluid-text-lg">{instructor.title}</h3>
                <div className="flex w-[329px] justify-between items-center">
                  <button type="button" className="flex items-center justify-center border transition duration-200 ease-in-out font-sans font-medium px-4 py-1 bg-yrci-red border-yrci-red text-white hover:bg-dark-teal hover:border-dark-teal hover:text-white active:bg-dark-teal active:border-dark-teal rounded-full w-max text-sm mt-auto group-hover:bg-light-gray">Read Bio</button>
                </div>
              </div>
            </a>
          </Link>
